...
while True:
    for task in main_vars.task_queue:
        ...
        if task['task_type'] == 'send_coords':
            try:
                send_coords(task['chat_id'], main_vars.bot, task['coords'])
            except Exception:
                main_vars.bot.send_message(task['chat_id'],
        ...
        if task['task_type'] == 'live_location':
            if task['chat_id']:
                ...
                session = main_vars.sessions_dict[main_vars.additional_ids[task['additional_chat_id']]]
            try:
                send_live_locations(chat_id, main_vars.bot, session, task['coords'], task['duration'])
            except Exception:
                main_vars.bot.send_message(task['chat_id'],
        ...
        if task['task_type'] == 'add_points_ll':
            if task['chat_id']:
                ...
                session = main_vars.sessions_dict[main_vars.additional_ids[task['additional_chat_id']]]
            try:
                add_custom_live_locations(chat_id, main_vars.bot, session, task['points_dict'], task['duration'])
            except Exception:
                main_vars.bot.send_message(task['chat_id'],
